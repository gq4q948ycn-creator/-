...
class TaskGeneratorApp:
    def __init__(self, root):
        ...
        ttk.Combobox(filter_frame, textvariable=self.filter_var, values=filter_options, state="readonly", width=15).pack(side=tk.LEFT, padx=5)
        self.filter_var.trace_add("write", self.update_history_list)

        # Кнопка генерации
        tk.Button(root, text="Сгенерировать задачу", bg="#4CAF50", fg="white", command=self.generate_task).pack(pady=10, fill=tk.X)

        # Поле текущей задачи
        self.current_task_label = tk.Label(root, text="Ваша задача появится здесь", font=('Arial', 12, 'bold'), wraplength=400)
    ...
    def add_new_task(self, task_text, task_type):
        """Добавление новой задачи."""
        if not task_text.strip():
            messagebox.showerror("Ошибка", "Поле задачи не может быть пустым!")
            return
        new_task = {"text": task_text.strip(), "type": task_type}
        self.tasks.insert(0, new_task)  # Добавляем в начало списка
        self.save_tasks()  # Сохраняем в файл
        self.update_history_list()  # Обновляем интерфейс
